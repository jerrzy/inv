...
class OptionApplication {
    ...
    async loadOptions() {
        const tickers = await this.tickerRepository.getTickers(this.hasOption, 'VUG');
    ...
    async #loadOptionData(ticker, optionType) {
        ...
        try {
            ...
            const quoteDate = Utility.formatDate(quoteDatetime);
    ...
    flatOptionObjs(optionObj, optionType) {
        ...
        for (let key in chainObj) {
            ...
            for (let strike in curObj) {
                ...
                detail.daysToExpire = daysToExpire;
    ...
    async sweepOptions() {
        const callCollection = await this.optionRepository.getOptionCollection('call');
        const optionArr = [];
        await callCollection.find({}).each(async (err, doc) => {
            optionArr.push({
                _id: doc._id,
                symbol: doc.symbol,
        ...
        for(let i = 0; i < optionArr.length; i++) {
            ...
            const quoteDate = doc.insert_date.split(' ')[0];
            // doc.
            const ret = await callCollection.updateOne({ _id: id }, { $set: { quote_date: quoteDate } });
            console.log(ret);
        }
        console.log();
    }
